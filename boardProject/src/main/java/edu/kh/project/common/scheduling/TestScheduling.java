 ...
 *  
 *  [설정 방법]
 *  1. 프로젝트명 Application.java 파일에
 *     @EnableScheduling 어노테이션 추가
 *     
 *  2. 스케쥴러 코드를 작성할 별도 클래스를 작성한 후 Bean으로 등록
 *     -> @Component 어노테이션 작성
 *     
...
@Component // 빈 등록 -> 스프링이 자동으로 스케쥴링 코드를 수행하게 함
@Slf4j
public class TestScheduling {
	...
	// [특수문자 별 의미]
	// * : 모든 수
	// - : 두 수 사이의 값 ex) 10-15 => 10이상 15 이하
	// , : 특정 값을 지정  ex) 3,6,9,12 => 3분 6분 9분 12분에 동작
	// / : 값 증가.        ex) 0/5 => 0부터 5씩 증가할때마다 동작
	...
	//         cron="초 분 시 일 월 요일 [년도]"
	// @Scheduled(cron="0 * * * * *") // 매 0초 마다 수행 (1분마다)
	// @Scheduled(cron="0/10 * * * * *") // 0부터 10초마다 수행
	// @Scheduled(cron="59 59 23 * * *") // 매일 자정마다 수행
